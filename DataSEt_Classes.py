...
from Label_Estimate_Helper_Functions import draw_filter, create_seed, Get_Upper_Lower_boundaries
class SegThorDS(Dataset):
    """Segthor dataset.

    :param root_dir: the directory containing the ROOT dataset for SegTHOR Dataset
    just a 
    """
        ...
        for root_path, _, files in os.walk(os.path.join(self.root_dir, typ, image_dir), topdown=False):
            if len(files) > 1:
                for file in files:
                    ...
                    gt_filename = self._build_train_input_filename(root_dir, patient_path, 'mask', dataset_type)
                    self.filename_pairs.append((input_filename, gt_filename))
...
class WMH(Dataset):
    '''
    CLass for the WHite matter hyperintensity dataset
    '''
    def __init__(self, root_dir, typ=None):
        ...
        self.filename_pairs = []
        #assert typ != None
        for root_path, _, files in os.walk(os.path.join(self.root_dir, typ), topdown=False):
            if len(files[0]) > 1:
                for file in files[0]:
                    patient_path = os.path.join(root_path, file)
                    input_filename = self._build_train_input_filename(root_dir, patient_path)
    ...
    def _build_train_input_filename(root_path, patient_path, im_type='img'):
        ...
        if im_type == 'img':
            ...
            return os.path.join(base_gt_path, '{}_manual1.gif'.format(basename.split('_training')[0]))
...
class SegTHor_2D_TrainDS(Dataset):
    def __init__(self, root_dir, transform=None, ds_mode='train', size=400, seed_div = 4, organ_n = 2):
        ...
        if ds_mode == 'train':
            self.train_dir = os.path.join(root_dir, h5_file.format('val'))
        elif ds_mode == 'val':
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(self.dir, "r") as self.f:
            ...
            ground_truth = torch.tensor(gt)
            bbox = draw_filter(np.array(self.bboxes_coord), 'bbox',
                                    self.organ_n+1, img.shape[1])
            #bbox = (bbox_mask == 255) * 1.0
            bbox_v = draw_filter(np.array(self.bboxes_coord), 'bbox',
                                    self.organ_n+1, img.shape[1])
            ...
            #seed = torch.tensor(create_seed((img.shape[1], img.shape[1]), self.bboxes_coord, self.seed_div, n_organ = self.organ_n+1))
            mask_distmap, mask_onehot = Get_DistMap(gt)
            #inner = (np.array(self.f['inner'][index]) == 255) * 1.0
            #outer = (np.array(self.f['outer'][index])== 255) * 1.0
            #bbox_dist = Get_DistMap(seed)[0]
            #Bound_U, _ = Get_DistMap(outer)
            #Bound_L, _ = Get_DistMap(inner)
            '''
            for i in range(0,4):
